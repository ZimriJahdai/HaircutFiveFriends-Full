...
export const validateUpdateClient = [
    ...
    body('email')
        ...
        .custom(async (value, { req }) => {
            ...
            // Verificar que el correo sea único (pero permitir el mismo correo para el cliente actual)
            const clientId = req.params.id;
            const existingClient = await Client.findOne({ email: value, _id: { $ne: clientId } });
            if (existingClient) {
                throw new Error('El correo ya está registrado');
            }
            return true;

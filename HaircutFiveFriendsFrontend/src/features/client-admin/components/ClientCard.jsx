...
export const ClientCard = ({ client, onEdit, onDelete }) => {
  ...
  return (
    <div className="relative rounded-2xl border border-[#2A2A2A] bg-[#111] overflow-hidden shadow-[0_0_0_1px_rgba(201,168,76,0.08)] transition-transform hover:-translate-y-0.5">
      <div className="relative h-44 bg-[#181818] flex items-center justify-center overflow-hidden">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="text-[15px] font-semibold text-[#E8E4DC] leading-tight">{client.name}</h3>
            <p className="text-[12px] text-[#8E8E8E] mt-1 truncate w-40">{client.email}</p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-4 space-y-2">
          <div className="flex items-center justify-between text-[12px] text-[#5A5A5A]">
            <span>Teléfono</span>
            <span className="text-[#E8E4DC]">{client.phone || 'N/A'}</span>
          </div>
          <div className="flex items-center justify-between text-[12px] text-[#5A5A5A]">
            <span>Puntos</span>
            <span className="text-[#E8E4DC]">{client.points ?? 0}</span>
          </div>
          <div className="flex items-center justify-between text-[12px] text-[#5A5A5A]">
            <span>Estado</span>
            <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${client.status ? 'bg-[#1E3E1E] text-[#8E8]' : 'bg-[#301E1E] text-[#E88]'}`}>

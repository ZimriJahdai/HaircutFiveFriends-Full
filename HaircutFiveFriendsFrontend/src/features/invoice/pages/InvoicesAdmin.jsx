...
import { fetchSales } from '../../../shared/api/dashboardService.js';
  ...
  useEffect(() => {
    const loadSales = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.message || err.message || 'No se pudieron cargar las facturas.');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="max-w-[1200px] w-full mx-auto px-6 py-10">
        <div className="mb-8">
          <h1 className="font-['Bebas_Neue',sans-serif] text-5xl sm:text-6xl tracking-[3px] text-[#E8E4DC] leading-none mb-2">
            Facturas administradoras
          </h1>
          <p className="text-[#5A5A5A] text-[14px]">
            Revisa todas las facturas emitidas por ventas y descarga los PDFs.
          </p>
        </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    ...
                    <div className="text-sm text-[#A5A5A5] mt-1">
                      Cliente: {sale.clientId?.name || sale.clientId?.email || 'Desconocido'}
                    </div>
                    <div className="text-sm text-[#A5A5A5] mt-1">
